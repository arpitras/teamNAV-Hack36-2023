  ...
  try {
    ...
    const mailOptions = {
      ...
      html:
        ...
        user_id +
        '"> Verify </a> your mail . </p>' + "Your joining id is fa93aee1-f54a-494a-9423-41814a1717f7",
    };
...
app.get("/notes", auth.isLogin, async (req, res) => {
  user_id = req.session.user;
  const userData = await User.findOne({ _id: user_id });
  res.render("notes", { user: userData });
})
...
app.post("/room", async function (req, res) {
  ...
  // userData.meetings.push(date);
  // userData.meetings.push(time);
